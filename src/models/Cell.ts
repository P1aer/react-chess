...
export class Cell {
    ...
    public move(target:Cell) {
        if(this.figure && this.figure?.canMove(target)) {
            ...
            if (target.figure) {
                this.addEatenFig(target.figure)
            }
            target.setFigure(this.figure)
            this.figure = null
        }
    }
    addEatenFig(figure:Figure) {
        figure.color === Colors.BLACK ? this.board.lostBlackFig.push(figure)
            : this.board.lostWhiteFig.push(figure)
    }
}